jQuery(document).ready(function () {
    ...
    jQuery('#permalink-manager').on('click', '.permalink-manager.redirects-panel #permalink-manager-new-redirect', function () {
        ...
        jQuery("input[data-index]", new_row).attr("data-index", last_key).attr("name", function () {
    ...
    function permalink_manager_duplicate_check(custom_uri_input, nonce) {
        // Set default values
    ...
    if (jQuery('#uri_editor .custom_uri').length > 0) {
        let uri_editor_nonce = jQuery('#uri_editor #uri_editor_nonce').val();
        permalink_manager_duplicate_check(false, uri_editor_nonce);
    }
